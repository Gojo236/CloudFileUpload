...
export default function StorageDistribution() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['filesFolder'], queryFn: fetchData })
  ...
  function fileDataFormatter(docs: IDoc[]) {
    ...
    result.reduce((acc, it) => { return it.size + acc }, 0);
  ...
  return (
    <BarChart
      ...
      series={[{ dataKey: 'size', label: `Storage Used ${dataSet.reduce((acc, it) => { return it.size + acc }, 0).toPrecision(2)} MB`, valueFormatter }]}
      layout="horizontal"
      grid={{ vertical: true }}
      {...chartSetting}
    />
